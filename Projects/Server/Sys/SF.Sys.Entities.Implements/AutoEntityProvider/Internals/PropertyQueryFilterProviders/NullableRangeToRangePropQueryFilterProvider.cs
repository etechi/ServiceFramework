...
{
	public class NullableRangeToRangePropQueryFilterProvider : TwoPropQueryFilterProvider
	{
		class Filter<T> : PropQueryFilter<QueryRange<T>> where T: struct, IComparable<T>
		{
			...
			{
				return ContextQueryableFilters.GetNullableRangeFilterExpression(value, prop1,prop2);
			}
		}
		...
		protected override bool MatchType(Type DataValueType1, Type DataValueType2, Type PropValueType)
		{
			if (!PropValueType.IsGenericTypeOf(typeof(QueryRange<>)))
				return false;
			if (DataValueType1.IsGenericTypeOf(typeof(Nullable<>)) && DataValueType1.GenericTypeArguments[0] != PropValueType.GenericTypeArguments[0] )
				return false;
			if (DataValueType2.IsGenericTypeOf(typeof(Nullable<>)) && DataValueType2.GenericTypeArguments[0] != PropValueType.GenericTypeArguments[0])
				return false;
			return true;
		}
