...
namespace SF.Common.TextMessages.Management
{
	...
	public class EntityMsgLogger : DefaultMsgLogger
	{
		...
		class ActionLogger : IMsgActionLogger
		{
			...
			public IDataSet<DataModels.MsgActionRecord> ActionRecords { get; set; }
			...
			public async Task<string> Add(MsgSendArgument Arg, Func<Task<string>> Action)
			{
				Count++;
				var id = await IdentGenerator.GenerateAsync<DataModels.MsgActionRecord>();
				var rec = new DataModels.MsgActionRecord
				{
					Args = Json.Stringify(Arg.Arguments),
					Content = Arg.Content,
					Id = id,
					UserId = Arg.TargetId,
					MsgRecordId = MsgRecordId,
					ServiceId = Arg.MsgProviderId,
					Time = Time,
					TrackEntityId = TrackEndityId,
					Title = Arg.Title,
					Target = Arg.Target
				};
		...
		public override async Task<long> Add(long? targetUserId, Message message, Func<IMsgActionLogger, Task<long>> Action)
		{
			return await ScopedDataContext.Use(async ctx => {
				...
				var msgRecords = ctx.Set<DataModels.MsgRecord>();
				var rec=new DataModels.MsgRecord
				{
					Id = id,
					UserId = targetUserId,
				...
				var al = new ActionLogger
				{
					...
					ActionRecords = ctx.Set<DataModels.MsgActionRecord>()
				};
				try
				{
