...
const attrs: { [index: string]: AttrInitializer} = { 
    ["SF.Metadata.LayoutAttribute"](item,values) {
    ...
    ["SF.Metadata.PasswordAttribute"](item, values) {
        if (item.type != "string")
            throw "Password标记必须应用于字符串字段";
        item.componentProps.type = "password";
    },
    ["SF.Metadata.DateAttribute"](item, values) {
    ...
    "SF.Metadata.RelationAttribute"(item, values) {

    },
    "SF.Metadata.UserRelationAttribute"(item, values) {

    }, 
    "SF.Metadata.HtmlAttribute"(item, values, ctx) {
        if (item.itemType != FI.FormItemType.Field)
            throw "图片只能是简单字段:" + item.formField;
        (item as FI.FormField).editor = "rich-editor";
    },
    "SF.Metadata.MultipleLinesAttribute"(item, values, ctx) {
        if (item.type!="string")
            throw "多行标志只能是文本字段:" + item.formField;
        item.componentProps.rows = 4;
    },
    "SF.Metadata.CopyableAttribute"(item, values, ctx) {
        if (item.type != "string")
            throw "可复制标志只能是文本字段:" + item.formField;
        item.componentProps.copyable = true;
    },
    
    "SF.Metadata.ImageAttribute"(item, values,ctx) {
        if (item.itemType != FI.FormItemType.Field)
            throw "图片只能是简单字段:" + item.formField;
        ...
        if (ctx.pathEntities.length >= 2) {
            ...
            if (!top2Type || !Meta.isArrayType(top2Type) ||
                ...
                top2.Attributes.filter(a =>
                    a.Type == "SF.Metadata.ArrayLayoutAttribute" &&
            ...
            if (!eType || ctx.lib.allTypeProperties(eType)
                .filter(p =>
                    ...
                    p.Attributes.filter(a =>
                        a.Type == "SF.Metadata.IgnoreAttribute"
                    ).length == 0
                ).length > 1)
                return;
    ...
    "SF.Metadata.TreeNodesAttribute"(item, values) {
        if (item.itemType != FI.FormItemType.Array)
            throw "TreeNodes必须在数组上使用";
        item.editor = "tree-editor";
        item.formField += "[]"; 
    },
    "SF.Metadata.TableRowsAttribute"(item, values) {
        if (item.itemType != FI.FormItemType.Array)
    ...
    "SF.Metadata.ArrayLayoutAttribute"(item, values) {
        if (item.itemType != FI.FormItemType.Array)
    ...
    "SF.Metadata.PropertyTypeAttribute"(item, values) {
        if (item.itemType != FI.FormItemType.Field)
