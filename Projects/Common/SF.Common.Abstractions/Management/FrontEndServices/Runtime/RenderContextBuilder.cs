...
namespace ServiceProtocol.Biz.UIManager.Runtime
{
	class RenderContextBuilder 
	{
		...
		public RenderContextBuilder(Logging.ILogger Logger, DI.IDIProviderResolver<IDataProvider> DataProviderResolver)
		{
			this.Logger = Logger;
			...
			)
		{
			...
			if (content.ContentId != 0 && ContentLoader!=null)
			{
				...
				if (!string.IsNullOrEmpty(ctn.ProviderType) && LoadData)
				{
					...
					try
					{
						dataProvider = DataProviderResolver.Resolve(ctn.ProviderType);
