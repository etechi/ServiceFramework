...
namespace SF.Core.ManagedServices.Runtime
{
	class ManagedServiceFactoryManager : IManagedServiceFactoryManager,IManagedServiceConfigChangedNotifier
	{
		...
		class ManagedServiceFactory : IManagedServiceFactory
		{
				...
				)
			{
				...
				lock (this)
				{
					...
					if (Id == "default")
					{
						ImplementType = ServiceMetadata.ManagedServices.Get(ServiceType)[0].Type;
						CreateArguments ="{}";
					}
					else
					{
						...
						CreateArguments = cfg.CreateArguments;
					}
		...
		string ResolveDefaultService(IServiceProvider ServiceProvider, Type type, string Id)
		{
			...
			if (Id == null)
			{
				var list = ServiceMetadata.ManagedServices.Get(type);
				if(list.Count!=1)
