			...
			)
		{
			...
			void InitCopyRequiredPaths() { 
				...
				void DetectCopyRequiredPath(Type t)
				{
					...
					try
					{
						if (t.IsArray)
							t = t.GetElementType();
						else if (t.GetTypeInfo().IsGenericType && t.GetGenericTypeDefinition() == typeof(IEnumerable<>))
			...
			Expression CopyExpression(Type Type, Expression src, Expression PropPathExpr,string PropPath)
			{
				...
				else
				{
					...
					if (svcType == Internal.ServiceType.Managed)
						return ManagedServiceResolveExpression(Type, PropPathExpr);
					else if (svcType == Internal.ServiceType.Normal)
